    ...
    $.fn.compliDocumentAlt = function () {
        ...
        function checkDocumentDetailsDirtyStatus(container) {
            if (container != null && container.length > 0) {
                var saveBtn = container.find(documentElementSelectors.buttons.DocumentDetailsSave);
                if (saveBtn.length > 0) {
                    if (isContainerFieldsDirty(container) == true) {
                        saveBtn.removeClass(documentElementClasses.DisabledButtonLink);
                        saveBtn.off('click');
                        saveBtn.on('click', onDocumentDetailsSaveBtnClick);
                    } else {
                        saveBtn.addClass(documentElementClasses.DisabledButtonLink);
                        saveBtn.off('click');
                        saveBtn.on('click', onDisabledButtonClick);
                    }
                }
            }
        }
        ...
        function checkDocumentRevisionDirtyStatus(container) {
            if (container) {
                var saveBtn = container.find(documentElementSelectors.buttons.DocumentRevisionDetailsSave);
                if (saveBtn.length > 0) {

                    if (isContainerFieldsDirty(container) == true) {
                        saveBtn.removeClass(documentElementClasses.DisabledButtonLink);
                        saveBtn.off('click');
                        saveBtn.on('click', onDocumentRevisionSaveBtnClick);
                    } else {
                        saveBtn.addClass(documentElementClasses.DisabledButtonLink);
                        saveBtn.off('click');
                        saveBtn.on('click', onDisabledButtonClick);
